    ...
    synchronized private SaslClient createSaslClient(final String servicePrincipal,
        ...
        try {
            ...
            if (subject.getPrincipals().isEmpty()) {
                ...
                // "leader-sasl-md5" is a hard-wired 'domain' parameter shared with zookeeper server code (see ServerCnxnFactory.java)
                saslClient = Sasl.createSaslClient(mechs, username, "zookeeper", "leader-sasl-md5", null, new ClientCallbackHandler(password));
                return saslClient;
            }
            else { // GSSAPI.
