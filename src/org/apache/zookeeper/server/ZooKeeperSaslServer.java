...
public class ZooKeeperSaslServer {
    ...
    private SaslServer createSaslServer(final Login login) {
        synchronized (login) {
            ...
            if (subject != null) {
                // server is using a JAAS-authenticated subject: determine service principal name and hostname from leader server's subject.
                if (subject.getPrincipals().size() > 0) {
                    ...
                    try {
                        SaslServer saslServer = Sasl.createSaslServer("DIGEST-MD5","zookeeper","leader-sasl-md5",null, login.callbackHandler);
                        return saslServer;
                    }
                    catch (SaslException e) {
