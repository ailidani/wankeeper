...
public class ClientCnxn {
    ...
    class SendThread extends ZooKeeperThread {
        ...
        private HashMap<Integer, ByteBuffer> packetsOutofOrder = new HashMap<>();
...
//                            + packet );
                    ...
                    packetsOutofOrder.put(replyHdr.getXid(), incomingBuffer.duplicate());
                ...
                finishPacket(packet);
                while (pendingQueue.size() != 0 && packetsOutofOrder.containsKey(pendingQueue.peek().requestHeader.getXid())) {
                    Packet pending = pendingQueue.remove();
                    ByteBuffer buffer = packetsOutofOrder.remove(pending.requestHeader.getXid());
                    ByteBufferInputStream bbis2 = new ByteBufferInputStream(buffer);
                    BinaryInputArchive bbia2 = BinaryInputArchive.getArchive(bbis2);
                    ReplyHeader replyHdr2 = new ReplyHeader();
                    replyHdr2.deserialize(bbia2, "header");
                    pending.replyHeader.setXid(replyHdr2.getXid());
                    pending.replyHeader.setErr(replyHdr2.getErr());
                    pending.replyHeader.setZxid(replyHdr2.getZxid());
                    if (replyHdr2.getZxid() > 0) {
                        lastZxid = replyHdr2.getZxid();
                    }
                    if (pending.response != null && replyHdr2.getErr() == 0) {
                        pending.response.deserialize(bbia2, "response");
                    }
                    if (LOG.isDebugEnabled()) {
                        LOG.debug("Reading reply sessionid:0x" + Long.toHexString(sessionId) + ", packet:: " + pending);
                    }
                    finishPacket(pending);
                }
            }
        }
