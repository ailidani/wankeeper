...
then
  ...
  then
    ...
    #   do a local attach
    ZOOMAIN="-Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.local.only=$JMXLOCALONLY org.wankeeper.server.WanKeeperMain"
  else
    if [ "x$JMXAUTH" = "x" ]
    then
      JMXAUTH=false
    ...
    ZOOMAIN="-Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.port=$JMXPORT -Dcom.sun.management.jmxremote.authenticate=$JMXAUTH -Dcom.sun.management.jmxremote.ssl=$JMXSSL -Dzookeeper.jmx.log4j.disable=$JMXLOG4J org.wankeeper.server.WanKeeperMain"
  fi
else
    echo "JMX disabled by user request" >&2
    ZOOMAIN="org.wankeeper.server.WanKeeperMain"
fi
...
if [ -e "$ZOOBIN/../libexec/Env.sh" ]; then
  . "$ZOOBINDIR"/../libexec/Env.sh
else
  . "$ZOOBINDIR"/Env.sh
fi
...
if [ -z "$ZOOPIDFILE" ]; then
    ...
    ZOOPIDFILE="$ZOO_DATADIR/wankeeper_server.pid"
else
